...
from source.agents.agentv2.database.client_windows import DatabaseClient
    ...
    for i in range(len(text_json)):
        ...
        if query_results == []:
            failed_orders.append(
                [{"name": search_string, "error_type": "MISSING_ITEM"}]
            )
        elif (
            len(query_results) == 2
            and query_results[0]["_additional"]["score"]
            == query_results[1]["_additional"]["score"]
            and query_results[0]["name"] != query_results[1]["name"]
        ):
            ...
            if not square_order.order_ongoing:
                success = square_order.create_order(
                    query_results[0][
                        "item_id"
                    ],  # the [0] index is bc we want the most similar vector.
            ...
            # add additional items to order
                ...
                success = square_order.add_item_to_order(
                    query_results[0]["item_id"], text_json[i]["quantity_ordered"], note
                )
            # save failed orders
            if not success:
