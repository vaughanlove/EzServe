...
class AutoServe:
    ...
    def text_to_speech(self, path, text: str):
        ...
        with tempfile.NamedTemporaryFile(
            dir=path, delete=True, suffix=".wav"
        ) as temp_wav_file:
            ...
            try:
                with sf.SoundFile(temp_speaker_wav_path) as f:
                    ...
                    stream = sd.OutputStream(
                        ...
                        callback=callback,
                        finished_callback=playback.set,
                    )
                    with stream:
    ...
    def error_validation(self, item):
        ...
        if item[0]["error_type"] == "MISSING_ITEM":
            self.text_to_speech(
                "app/source/audio/audio_out/",
                f"Sorry, {item[0]['name']} is not on our menu. Could you please re-specify exactly what you want?",
            )
        elif item[0]["error_type"] == "SCORE_MATCH":
            self.text_to_speech(
                "app/source/audio/audio_out/",
                f"We arn't sure if you wanted a {item[0]['name']} or a {item[1]['name']}, could you specify?",
            )
        elif item[0]["error_type"] == "SQUARE_CALL":
            self.text_to_speech(
                "app/source/audio/audio_out/",
                f"Something strange happened on Square's end. We are looking into it.",
            )
    ...
    async def failed_order_execution(self, item):
        ...
        # prompt the agent
        agent_response, language = await self.record(
            "app/source/audio/human_input_in/", recording_flag
        )
        ...
        try:
            failed_orders = json.loads(
                agent_response.split("Failed Orders:")[-1].strip()
            )
        except:
        ...
        logger.info(
            f"(failed_order_execution) - translated response: {translated_response}"
        )
    ...
    async def order_execution(self):
        ...
        while not shutdown_flag.is_set():
            ...
            agent_response, language = await self.record(
                "app/source/audio/audio_in/", recording_flag
            )
            ...
            # Asyncronous call speaker
            logger.info(
                f"(order_execution) - translated response: {translated_response}\n"
            )
            ...
            self.text_to_speech(
                "app/source/audio/audio_out/",
                "Feel free to continue ordering, the items you've ordered are on their way.",
            )
    ...
    def __init__(self, trace=False, verbose=True) -> bool:
        ...
        # self.text_to_speech(
        #     "app/source/audio/audio_out/",
        #     f"Hello! I am EzServe and will be taking your order. Please ask about the menu, place orders, and request to checkout when you're done.",
        # )
